<?php
class AdminSlideshow extends Module_Base  
{
...
//			$arrData['gallery'] = _slide::get_gallery_by_id($arrData[$this->_prefix.'id'],$this->type);
		...
		if(checkMultiLang())
			$this->quickForm->addElement("select",$this->_prefix."lang_id",array("style"=>"width:200px"), array('options' => parent::getAssocLang(), 'label' => 'Ngôn ngữ'));


//		$arrPanrent = $this->getParent(0,0);
		$name = $this->quickForm->addElement('text', $this->_prefix.'title')->setLabel("Name");
		$this->quickForm->addElement('imagepopup', $this->_prefix.'photo', array("data-folder"=>"Images"), array('label' => 'Ảnh đại diện:'));
//		$dezhub->addElement('text', $this->_prefix.'code')->setLabel("Email");
		$this->quickForm->addElement('text', $this->_prefix.'description')->setLabel("Thứ tự");
...
//		parent::insertFormMeta();
		...
		if($task=='edit'){
			$this->quickForm->addElement('datetime',$this->_prefix. 'updatetime')->setLabel("Ngày cập nhật");
			$this->quickForm->addElement('datetime',$this->_prefix. 'createtime')->setLabel("Ngày khởi tạo");
		}
	...
	if (parent::validate()) {
		...
		$display = parent::_smatyrender();
		$this->s->assign("FormData",$display);
		$this->s->display("form.tpl");
		//parent::displayForm();	
