...
{
	public $quickForm,$db,$s,$submit_url,$slideshow,$option;
	...
	function buildForm ( $task, $arrData = array() )
	{
		...
		if($task=='edit'){
			...
			$arrData[$this->_prefix."price"] = $this->option->get_option_by_id($arrData[$this->_prefix.'id'],$this->type."_price");
...
//		$dezhub->addElement('inputcurrency',$this->_prefix. 'price',array("style"=>"width:200px"))->setLabel("Giá cũ");
		$dezhub->addElement('inputcurrency',$this->_prefix. 'price',array("style"=>"width:200px"))->setLabel("Giá");
		...
		$editor = $dezhub -> addElement ('editor' ,$this->_prefix. 'description',
		array("style"=>"width:300px","data-type"=>"small"))->setLabel("Miêu tả ngắn gọn");
...
//		_slide::buidForm($this->quickForm,$arrData[$this->_prefix.'id']);
	...
	if (parent::validate()) {
			// group
				...
				$price = preg_replace("/[_.VND]+/","",$form[$this->_prefix.'price']);
				$this->option->update_option($news_id,$price,$this->type."_price");
