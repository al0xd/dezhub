...
{
	public $quickForm,$db,$s,$type,$option_class,$_prefix,$BaseSlide,$BaseOption;
	function __construct(){
		
		parent::__construct();
		parent::setTitle();
		$this->option_class = new BaseOption();
	}
	...
	function listItem()
	{
		...
		if(checkMultiLang())
			...
			$this->grid->addTask(
				array(
					"task"=>"build",
					"url"=>SITE_URL."index.php?mod=build&ajax=true",
					"text"=>"Thiết kế",
					"icon"=>"icon-building",
					"tooltip"=>"Thiết kế trang",
				)
			);
	...
	function buildForm ( $task, $arrData = array() )
	{
		if($task=='edit'){
			...
			$arrData[$this->_prefix."skin"] = $this->option_class->get_option_by_id($arrData[$this->_prefix.'id'],$this->type."_skin");
			$arrData[$this->_prefix."comment"] = $this->option_class->get_option_by_id($arrData[$this->_prefix.'id'],$this->type."_comment");
		}
...
//		$this->BaseSlide->buidForm($this->quickForm,$arrData[$this->_prefix.'id']);
		parent::insertFormMeta();
		$config = parent::setFormFieldSet("Cấu hình bài viết");
//		$config->addElement("select",$this->_prefix."skin",array("style"=>"width:200px"), array('options' =>$this->skin, 'label' => 'Giao diện'));
//		$config->addElement(
//			'checkbox', $this->_prefix.'comment', null, array('content' => '', 'label' => 'Cho phép bình luận')
//		);
		$config->addElement(
	...
	if (parent::validate()) {
				$form_val = $this->quickForm->getValue();
			// group
...
//				$this->option_class->update_option($news_id,$form_val[$this->_prefix.'skin'],$this->type."_skin");
//				$this->option_class->update_option($news_id,$form_val[$this->_prefix.'comment'],$this->type."_comment");
//				$this->BaseSlide->update($news_id,$this->type,$_POST['gallery']);
